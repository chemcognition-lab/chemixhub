...
def main(
    ...
    # Split Loader
    split_loader = SplitLoader(split_type="temperature")
...
if __name__ == "__main__":
    ...
    config.root_dir = os.path.abspath(f"../results/temp_split/baseline/{experiment_name}/{config.dataset.name}/{config.dataset.property.lower().replace(' ', '_')}")

    if args.physics_based:
        config.mixture_model.regressor.type = "physics_based"
        config.root_dir = config.root_dir.replace("baseline", "physics_based")
